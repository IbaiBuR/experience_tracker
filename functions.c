...
void write_BLexp_entry_toTxTFile(FILE* experience, FILE* output)
{
    ...
    {
        fprintf(output, "Key: %x, Move: %s, Score: %d, Depth: %d, Performance: %d\n",
...
char *  move_from_decimal(int32_t move)
{
    ...
    sprintf(result, "%c%d%c%d", 'a' + converted.first_letter, converted.first_rank + 1, 'a' + converted.second_letter, converted.second_rank + 1);
